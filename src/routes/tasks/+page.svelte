<script lang="ts">
  ...
  import { createTask, readTasks } from "../../services/service";
          ...
          <li class="flex items-center justify-between gap-x-6 py-5">
            <div class="min-w-0">
              ...
              >
                ...
                <p class="whitespace-nowrap">{formatDate(task.createdDate)}</p>
...
{#if showModal}
  <Modal bind:open={showModal} title="Add Task">
    <form use:form>
      ...
      <div class="mb-6">
        <Label for="title" class="block mb-2">Description</Label>
        <Input id="title" size="sm" />
      </div>
      {#if saving}
        ...
        >
      {:else}
        <Button color="green" class="flex mx-auto align-middle" type="submit"
          >Save</Button
        >
      {/if}
    </form>
